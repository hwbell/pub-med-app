...
it('renders without crashing', async () => {
  let deepwrapper = mount(<CollectionForm {...someProps} />);
});
...
it('should set the highlightInd for selector buttons and show/hide the input accordingly', () => {
  const wrapper = shallow(<CollectionForm {...someProps} />);

  // the input should initially be present, since no exisiting collections are selected
  ...
  // switch it back and make sure the input appears
  expect(wrapper.state().highlightInd).toEqual(1);
...
it('should fire the submit function when "add to collection" button is clicked', () => {
  ...
  // have been called
  expect(deepWrapper.state().highlightInd).toEqual(null);
  expect(createNewStub.mock.calls.length).toEqual(1);
